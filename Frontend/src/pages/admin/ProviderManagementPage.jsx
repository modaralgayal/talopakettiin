...
export const ProviderManagementPage = () => {
  ...
  const fetchProvidersAndDomains = async () => {
    ...
    try {
      const [providersRes, domainsRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/api/user/get-all-providers`, { withCredentials: true }),
        axios.get(`${API_BASE_URL}/api/user/get-all-domain`, { withCredentials: true })
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const endpoint = type === 'email' ? '/api/user/add-provider-email' : '/api/user/add-provider-domain';
      const body = type === 'email' ? { email: value } : { domain: value };
      ...
      if (response.data.success) {
        ...
        // Refresh the lists after adding new item
        fetchProvidersAndDomains();
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24">
      <Helmet>
        ...
        <meta name="description" content="Admin Provider Management - Talopakettiin" />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700">Add Provider by:</label>
            ...
            <div>
              ...
              <div className="mt-1">
                <input
                  ...
                  placeholder={type === 'email' ? 'e.g. provider@example.com' : 'e.g. example.com'}
                  value={value}
            ...
            <div>
              ...
              >
                {isLoading ? 'Adding...' : `Add ${type === 'email' ? 'Email' : 'Domain'}`}
              </button>
            </div>
          </form>
          ...
          <div className="mt-12 space-y-8">
            {/* Providers List */}
            <div>
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Verified Provider Emails</h2>
              {isLoadingLists ? (
                ...
                <p className="text-gray-500">No verified provider emails found.</p>
              )}
            </div>

            {/* Domains List */}
            <div>
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Verified Provider Domains</h2>
              {isLoadingLists ? (
                ...
                <p className="text-gray-500">No verified provider domains found.</p>
              )}
            </div>
          </div>
        </div>
